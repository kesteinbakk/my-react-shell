...
export const PaperCard = forwardRef(function PaperCard({ title, subtitle, content, contentAlignX = 'left', contentAlignY = 'top', maxLines, tone, color, accentPlacement = 'top', footer, corner, watermark, image, size = 'md', onClick, hoverable, showDragHandle, dragHandle, dragHandleProps, dragHandleLabel, renderLink, className, style: styleProp, }, ref) {
    ...
    return (_jsxs("div", { ref: ref, className: cn('mrs-paper-card', hasAccent && `mrs-paper-card--accent-${accentPlacement}`, isHoverable && 'mrs-paper-card--hoverable', watermark && 'mrs-paper-card--watermark', image != null && 'mrs-paper-card--imaged', hasDragHandle && 'mrs-paper-card--draggable', corner != null && 'mrs-paper-card--cornered', renderLink && 'mrs-paper-card--linked', className), style: style, onClick: onClick, children: [renderLink
