...
export function InputField({ label, description, error, containerClassName, inputSize = 'md', fullWidth = false, className, onDebouncedChange, debounceMs = 500, onChange, saveStatus, onBlur, ...inputProps }) {
    ...
    const inputEl = (_jsx("input", { id: id, className: cn('mrs-field__input', inputSize !== 'md' && `mrs-field__input--${inputSize}`, isError && 'mrs-field__input--error', localStatus === 'saved' && 'mrs-field__input--saved-icon', localStatus === 'saving' && 'mrs-field__input--saving', className), "aria-invalid": isError || undefined, "aria-describedby": showError ? errId : showDesc ? descId : undefined, onChange: handleChange, onBlur: handleBlur, ...inputProps }));
    const wrappedInput = localStatus === 'saved' ? (_jsxs("div", { className: cn('mrs-input-wrapper', fullWidth && 'mrs-input-wrapper--full'), children: [inputEl, _jsx("span", { className: "mrs-input-icon-saved", children: _jsx(CheckIcon, {}) })] })) : (inputEl);
