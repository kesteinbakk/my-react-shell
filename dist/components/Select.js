 ...
 * `value` / `onValueChange`.
 */
export function Select({ options, value, onValueChange, placeholder = 'Select…', disabled, size = 'md', fullWidth = false, className, saveStatus, ...rest }) {
    const isError = saveStatus === 'error';
    return (_jsxs(RadixSelect.Root, { value: value, onValueChange: onValueChange, disabled: disabled, children: [_jsxs(RadixSelect.Trigger, { className: cn('mrs-select__trigger', `mrs-select__trigger--${size}`, fullWidth && 'mrs-select__trigger--full', saveStatus === 'saved' && 'mrs-select__trigger--saved', isError && 'mrs-select__trigger--error', className), "aria-invalid": isError || undefined, "aria-label": rest['aria-label'], children: [_jsx(RadixSelect.Value, { placeholder: placeholder }), _jsx(RadixSelect.Icon, { className: "mrs-select__chevron", children: chevron })] }), _jsx(RadixSelect.Portal, { children: _jsx(RadixSelect.Content, { className: "mrs-select__content", position: "popper", sideOffset: 4, children: _jsx(RadixSelect.Viewport, { className: "mrs-select__viewport", children: options.map((opt) => (_jsxs(RadixSelect.Item, { value: opt.value, disabled: opt.disabled, className: "mrs-select__item", children: [_jsx("span", { className: "mrs-select__indicator", children: _jsx(RadixSelect.ItemIndicator, { children: check }) }), _jsx(RadixSelect.ItemText, { children: opt.label })] }, opt.value))) }) }) })] }));
}
