...
const lastSingleByPath = new Map();
function isReturnFromDescendant(currentPath) {
 ...
 */
function getTabFromSearch(key, search) {
...
export function PageSections(props) {
    ...
    const initialId = () => {
        ...
        if (props.persistKey) {
            ...
            if (fromUrl && props.sections.some((s) => s.id === fromUrl))
                return fromUrl;
        }
    ...
    const setTabInUrl = (key, value) => {
        if (getTabFromSearch(key, search) === value)
            return;
        navigate({
            to: pathname,
    ...
    useEffect(() => {
        ...
        if (!fromUrl)
            return;
        if (!props.sections.some((s) => s.id === fromUrl))
            return;
        if (fromUrl === activeIdRef.current)
            return;
        setActiveId(fromUrl);
        ...
        if (mode !== 'single')
            scrollActiveIntoView(fromUrl, 'smooth');
    ...
    const prevActiveForUrl = useRef(undefined);
    useEffect(() => {
    ...
    function scrollActiveIntoView(id, behavior) {
    ...
    return (_jsxs("div", { className: `mrs-sections${props.fullHeight ? ' mrs-sections--full-height' : ''}${props.className ? ` ${props.className}` : ''}`, children: [_jsx("div", { ref: stripRef, className: "mrs-sections__strip", children: _jsx(SectionTabsStrip, { sections: props.sections, activeId: activeId, onTabClick: handleTabClick, showBaseline: props.showBaseline }) }), _jsx("div", { className: "mrs-sections__body", children: mode === 'list' ? (_jsx(SectionsListMode, { sections: props.sections, registerRef: registerRef, forceMountAll: forceMountAll, scrollRoot: scrollContainer, renderIcon: shell?.config.renderIcon })) : (_jsx(SectionsSingleMode, { sections: props.sections, activeId: activeId })) })] }));
}
