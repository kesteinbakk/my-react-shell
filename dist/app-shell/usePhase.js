...
export function usePhase() {
    ...
    if (shell.phase === null) {
        throw new Error('usePhase() requires a `phase` block in your defineShellConfig — none was declared.');
    }
    return asState(shell.phase);
}
/**
 * Read/drive the app-phase, tolerating its absence. Returns `null` outside
 * `<AppShell>` or when the config declares no `phase` block.
 */
export function usePhaseOptional() {
    const shell = useShellContextOptional();
    if (!shell || shell.phase === null)
        return null;
    return asState(shell.phase);
}
