...
What this is, the module set, and the model: [docs/concept.md](../concept.md).
...
- **The API reference is the single source of truth and always current.** Any change to
  ...
  [docs/specifications/api-reference.md](../specifications/api-reference.md) **in the same
  change**. It ships inside the package, so there is exactly one copy — keep it current.
...
- **A change isn't done until consumers pass.** An API-surface edit breaks every `link:`
  ...
  `dist/` once, then typecheck/verify all repos — and only then do docs. Never rebuild
  `dist/` then sit in a long docs pass with consumers on the old names.
- **Never hand release steps to the user.** Releasing the shell, themes, or a consumer is
  an agent responsibility, every time; follow
  [docs/maintainers/release-runbook.md](../maintainers/release-runbook.md) exactly.
