...
| `IconButton` | component | The canonical square **icon-only button** — a ghost button wrapping one icon glyph in a fixed square hit-area (the box scale, `--mrs-icon-btn-*`), lit on hover / `[data-state='open']` / focus. `size` (`sm·md·lg·xl`, default `md`), `active` (→ `aria-pressed`), `aria-label` (name a non-decorative button); native `<button>` props pass through and the `ref` is forwarded, so it drops into a Radix `asChild` trigger. The glyph size is the caller's — pair it with `ICON_BUTTON_GLYPH_PX[size]`. `DropdownMenu` renders it for its `iconTrigger`. |
