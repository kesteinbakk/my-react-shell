...
| `watermarkVariant` | `'faint'` | `'faint'` = the dimmed background mark; `'solid'` = the same glyph **full-opacity, full-color** at the same place and size (a clear icon, not a watermark). One slot, so a tile shows one or the other. |
