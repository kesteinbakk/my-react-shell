...
| `Input` | component | Un-opinionated native `<input>`. `invalid` (sets `aria-invalid` + error styling), `inputSize` (sm·md·lg; named so it never clashes with native `size`), `onDebouncedChange(value)` (fires `debounceMs` after the user stops typing; default 500 ms), `saveStatus` (visual status `'idle'`·`'pending'`·`'saving'`·`'saved'`·`'error'`), optional `label` (renders above the input); native input props pass through. **`required`** is shell-managed — it renders the red asterisk on the built-in `label` and sets `aria-required`, but does **not** set the native `required` attribute (no native validation bubble). **`validateOnBlur`** (default `false`) opts into shell-owned validation: an empty `required` field turns invalid (red border) once blurred and clears the moment a value is typed; OR-ed with the controlled `invalid`, which wins. |
| `Textarea` | component | Un-opinionated native `<textarea>`. `invalid` (sets `aria-invalid` + error styling), `onDebouncedChange(value)` (fires `debounceMs` after the user stops typing; default 500 ms), `saveStatus` (visual status `'idle'`·`'pending'`·`'saving'`·`'saved'`·`'error'`), optional `label` (renders above the textarea); native textarea props pass through. **`required`** / **`validateOnBlur`** behave exactly as on `Input` (red asterisk + `aria-required`, no native constraint; opt-in red-border-on-blur). |
...
| `InputField` | component | Full field: label + input + helper + error, a11y-wired (`htmlFor`/`aria-invalid`/`aria-describedby`). Spreads native input props; pass `error` to switch on error styling. `inputSize` (`sm`·`md`·`lg`, default `md`) matches the `Input` height/padding scale. `onDebouncedChange(value)` (fires `debounceMs` after the user stops typing; default 500 ms), `saveStatus` (visual status `'idle'`·`'pending'`·`'saving'`·`'saved'`·`'error'`). **`required`** renders the red asterisk on the label and sets `aria-required` (no native constraint); **`validateOnBlur`** (default `false`) turns an empty `required` field's border red once blurred (no message text — pass `error` for one), OR-ed with the controlled `error`, which wins. |
| `SegmentedControl` | component | Single-select `radiogroup` on a track; controlled via `value`/`onChange`; generic over value type. |
| `Select` | component | Opinionated select on Radix Select (keyboard nav, typeahead, portal); `options` list; controlled via `value`/`onValueChange`; `size` (`sm`·`md`·`lg`, default `md`) matches the `Input` height/padding scale; `saveStatus` (visual status `'idle'`·`'pending'`·`'saving'`·`'saved'`·`'error'`); optional `label` (renders above the select trigger); supports custom `className` and `style` on the trigger. **`required`** renders the red asterisk on the `label` and sets `aria-required` on the trigger (no native constraint); **`validateOnBlur`** (default `false`) turns the trigger border red once blurred with no value selected, clearing on selection. |
