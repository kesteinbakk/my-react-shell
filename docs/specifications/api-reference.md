...
| `Preview` | component | Controlled modal PDF viewer styled like a piece of paper on Radix Dialog. Uses full viewport height and auto width. Rendered using `react-pdf`. `open`/`onOpenChange`, `title`, `file` (string url, `File`, or `Blob`), `headerContent`. Provides native print functionality. Requires `react-pdf` as a peer dependency. |
