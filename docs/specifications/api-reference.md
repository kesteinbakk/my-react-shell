...
| `StatCard` | component | Self-contained φ-framed KPI/status card — a fixed-width golden-ratio card (`height = width / φ`); `size` default `md` ≈312px, four to a `wide` (1440px) row. Title + subtitle, an optional corner accent medallion (SVG arc-ring, `value`/`max` both mandatory), a stats row (any item can render as its own circle-or-arc medallion via `stats[].medallion`), and a structured `footer` or freeform `lower`. Accent stripe (`accentPlacement` top/left) + medallion tint driven by `tone` (semantic tokens) or a raw CSS `color`. `variant` (`'warning'`·`'danger'`) overrides tone, forces ⚠️ watermark. Optional left-edge completion gauge (`sideBarCompleteness`, red→amber→green) that coexists with the top stripe, or drives the whole accent's color via `topStripeFollowsGauge`. Optional emoji or ReactNode `watermark`. Hover-lift via `onClick`/`hoverable`. Optional `info` button (blue ⓘ, lower-right or lower-left corner) that opens a dialog with a title, optional description, and optional content (plain string or numbered sections). |
...
accent **medallion** (arc-ring), a stats row (any item can render as its own medallion via
`stats[].medallion`), and a structured `footer` or freeform `lower`. `size` widths `sm` 240 ·
`md` 312 · `lg` 400 · `xl` 520 px (default `md`). Acts as a whole-card link via `renderLink`.
Medallion, gauge (`sideBarCompleteness`, `topStripeFollowsGauge`), `variant`, and worked
examples: [card guide → StatCard](../guides/card-grid.md#statcard).
...
| `medallion` | — | `{ value, max, size? }` — the top-right corner arc-ring; `value` and `max` are both **required** (no plain-circle mode at the corner). `size` defaults to `'lg'`; `'sm'` renders a smaller footprint. Smaller and pulled in tighter to the corner than a `stats[].medallion` item. |
...
| `stats` | — | `{ value, label?, max?, medallion? }[]` — data items. `label` → label above + number below. `max` → compact arc-ring. **Cannot combine `label` and `max` on the same item** (throws in dev), unless `medallion` is set. `medallion: true` renders the item as a circle (plain `value`+`label`) or arc-ring (when `max` is set) — the same treatment as the corner medallion — at a fixed, compact size; in this mode `label`+`max` can coexist (matching the corner medallion's own behavior: `label` is ignored when `max` is present). |
...
| `watermarkMode` | `'art'` | How a **ReactNode** `watermark` is laid out (ignored for a string/variant watermark). `'art'` = the existing self-sized illustration layer (e.g. a `DrawerMark`), centred + dropped below centre. `'glyph'` = a keyed icon glyph (a lucide `<svg>` / emoji span from a consumer icon kit) scaled and positioned to **mirror the string-emoji watermark**: oversized, faint, centred, without tilt. Use `'glyph'` when the watermark is a single icon rather than an illustration. |
