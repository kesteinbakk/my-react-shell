...
| `watermark` | — | Faint background watermark — centred horizontally and vertically. A **`string`** is an oversized emoji (e.g. `'🏆'`); a **`ReactNode`** (e.g. a `DrawerMark`) renders in an art layer behind the content and makes the card root a `mrs-reveal-host` (so a hover-reveal mark opens on card hover). Ignored when `variant` is set. |
