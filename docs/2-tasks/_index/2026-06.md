...
|------|------|--------|----------|-------------|---------|
| T001 | port-and-rebuild-on-shadcn | in-progress | — | **Filed 2026-06-15.** Stand up react-shell as the "everything above shadcn" React foundation, per `notes/react-framework-guide.md` §2/§4/§5. Do **not** port foundation-react's ~62 Base-UI/CVA primitives — shadcn supersedes them (and now supports the same Base UI layer). Instead host react-shell's shared shadcn registry (brand token values as a `registry:base` + the few bespoke composites shadcn lacks) and build the layer shadcn never provides: the app-shell (carrying the SolidJS foundation's app-shell-rules contracts), Convex/auth/theme providers (Convex Auth default, Better Auth scale-up), the i18n `t()` seam, and the `~/config` + no-silent-defaults contracts. Blocks every future react-shell consumer. **Refs:** react-framework-guide §2/§4/§5. | registry config + shadcn MCP, `src/shell/`, `src/providers/`, `src/i18n/`, `docs/guides/` (app-shell + contracts). |
