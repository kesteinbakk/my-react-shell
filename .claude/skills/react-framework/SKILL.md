...
- **Build = `tsc -b && vite build`** (Vercel runs `pnpm build`; type errors must
  gate the deploy).
- **Build on react-shell — don't rebuild it.** react-shell owns app-shell,
  ...
  primitive kit.
- **Reuse-first UI.** Browse react-shell's registry via MCP before building any
