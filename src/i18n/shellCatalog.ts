...
import commonNbNO from './locales/common-nb-NO.json'
import { flattenMessages, mergeMessages } from './translate'
...
export const DEFAULT_SHELL_LOCALE = 'en-US'

/** The shell's bundled chrome catalogs, per shipped locale (nested form). */
export const SHELL_CATALOG: Record<Locale, Messages> = {
  'en-US': commonEnUS as Messages,
  'nb-NO': commonNbNO as Messages,
}

/** The same catalogs pre-flattened to dotted keys — the fast path for `useShellText`. */
export const SHELL_CATALOG_FLAT: Record<Locale, FlatMessages> = Object.fromEntries(
  Object.entries(SHELL_CATALOG).map(([code, msgs]) => [code, flattenMessages(msgs)]),
)
 ...
 */
export function withShellCatalog(messages: Record<Locale, Messages>): Record<Locale, Messages> {
  const out: Record<Locale, Messages> = {}
  for (const code of Object.keys(messages)) {
    const base = SHELL_CATALOG[code] ?? SHELL_CATALOG[DEFAULT_SHELL_LOCALE]
    out[code] = mergeMessages(base, messages[code] as Messages)
  }
  return out
}
