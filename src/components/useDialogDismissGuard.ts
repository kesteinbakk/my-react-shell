 ...
 */
function interactionInsideNestedDialog(event: Event): boolean {
 ...
 */
function focusFellOutToRoot(event: Event): boolean {
  if (typeof document === 'undefined') return false
  const original = (event as CustomEvent<{ originalEvent?: Event }>).detail?.originalEvent
  const isFocusEvent =
    original != null &&
  ...
  if (!isFocusEvent) return false
  const target = (original as FocusEvent).target as Node | null
