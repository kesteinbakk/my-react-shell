...
function asState<T extends string>(runtime: ShellAppModeRuntime): ShellAppModeState<T> {
  return runtime as unknown as ShellAppModeState<T>
}
 ...
 */
export function useAppMode<T extends string = string>(): ShellAppModeState<T> {
  const shell = useShellContext()
  if (shell.appMode === null) {
    throw new Error(
      'useAppMode() requires an `appMode` block in your defineShellConfig — none was declared.',
    )
  }
  return asState<T>(shell.appMode)
}
 ...
 */
export function useAppModeOptional<T extends string = string>(): ShellAppModeState<T> | null {
  const shell = useShellContextOptional()
  if (!shell || shell.appMode === null) return null
  return asState<T>(shell.appMode)
}
