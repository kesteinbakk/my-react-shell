...
function asState<T extends string>(runtime: ShellPhaseRuntime): ShellPhaseState<T> {
  return runtime as unknown as ShellPhaseState<T>
}
 ...
 */
export function usePhase<T extends string = string>(): ShellPhaseState<T> {
  const shell = useShellContext()
  if (shell.phase === null) {
    throw new Error(
      'usePhase() requires a `phase` block in your defineShellConfig — none was declared.',
    )
  }
  return asState<T>(shell.phase)
}
 ...
 */
export function usePhaseOptional<T extends string = string>(): ShellPhaseState<T> | null {
  const shell = useShellContextOptional()
  if (!shell || shell.phase === null) return null
  return asState<T>(shell.phase)
}
